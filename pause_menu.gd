...
func _wobble(node: Node3D) -> void:
	...
	tween.tween_property(node, "rotation:z", deg_to_rad(2.0), 0.1)
	tween.chain().tween_property(node, "rotation:z", deg_to_rad(-1.5), 0.1)
	tween.chain().tween_property(node, "rotation:z", 0.0, 0.1)
